...
# Only use the real Ollama client when explicitly enabled via env var.
# This avoids network calls during imports/runs in environments without Ollama.
use_ollama = os.environ.get("USE_OLLAMA", "").lower() in ("1", "true", "yes")


def _ollama_reachable(host: str, port: int, timeout: float = 1.0) -> bool:
    try:
...
# If requested, perform a short health-check before instantiating the real client.
if use_ollama:
    ...
    try:
        ...
        ollama_port = 11434
    try:
        ollama_timeout = float(os.environ.get("OLLAMA_TIMEOUT", "1.0"))
    except Exception:
        ollama_timeout = 1.0

    if not _ollama_reachable(ollama_host, ollama_port, ollama_timeout):
        print(f"Ollama at {ollama_host}:{ollama_port} not reachable (timeout={ollama_timeout}s). Falling back to MockLLM.")
        use_ollama = False

if use_ollama and _OllamaLLM is not None:
    llm = _OllamaLLM(model="llama3")
elif use_ollama and _OllamaLegacy is not None:
